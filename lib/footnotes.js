...
function doFootnotes(file) {
  ...
  $("span.footnote").each(function (i, elem) {
    var link = $("a", elem).first();
    var index = $(link).text();
    var text = escapeHtml(lessjavascript.doLessJavascript($("#" + $(link).attr('href').slice(1)).html().trim()));
  ...
  if (file.snippet) {
    parsed_snippet("span.footnote").each(function (i, elem) {
      var link = $("a", elem).first();
      var index = $(link).text();
      var text = escapeHtml(lessjavascript.doLessJavascript($("#" + $(link).attr('href').slice(1)).html().trim()));
      var new_element = '<span class="badge footnote default-tooltip" ' +
