...
The file `userland/include/unistd.h` contains the user-level interface definition
...
`kern/include/syscall.h`. As you discovered in link:/asst/0/[ASST0], the integer codes for
...
setter for the address space that a process has been assigned. You will find an
example of how to use `proc_setas` by looking into the `runprogram` function.
As a final note, address space management is one of the core objectives of ASST3.
