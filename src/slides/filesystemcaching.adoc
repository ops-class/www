...
description: >
  Discussion of file system caching and its impacts on consistency, as well as
  journaling as a solution to this problem.
spelling_exceptions:
  - LFS
  - FFS
---
...
* Today: caching and consistency (maybe slightly short).
* Wednesday: FFS and LFS.
* Friday: RAID. Please read the paper and look at the online resources posted
on Piazza.
...
== (Aside) On Top of Spa-caches...

[.slider]
.How can the *disk cache* potentially get in our way here?
* Data that the operating system *thinks* is on disk may be stuck in a
*device cache* on lost on failure.

[.slider]
.How do we solve this problem?
. The device may support a command allowing the operating system to
flush its own internal cache.
. Or, the device may use a battery backup system to ensure that all
cached modifications are flushed in case the main power is removed.
(Some drives even harness the rotational energy of the disk platters
themselves to power shutdown. Fancy [.spelling_exception]#shmancy#.)
